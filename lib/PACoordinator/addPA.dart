...
class addPA extends StatefulWidget {
  const addPA({Key? key}) : super(key: key);

  @override
  _addPAState createState() => _addPAState();
}

class _addPAState extends State<addPA> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('PA').snapshots(),
                  ...
                  onPressed: () async {
                    ...
                    if (_validate == false) {
                      QuerySnapshot upmID_Stream =
                          await paInfo.where('upmid', isEqualTo: upmID).get();
                      ...
                      if (upmID_Stream != null) {
                        setState(() {
                          regPA(upm_id, role, fullName, image, faculty, dept,
